...
export default function Question() {
  ...
  const [showingCreateNewQuestionForm, setShowingCreateNewQuestionForm] =
    useState(false);
  const [questionCurrentlyBeingUpdated, setQuestionCurrentlyBeingUpdated] =
    useState(null);
    ...
    <div className="container">
      <div className="row min-vh-100">
        <div className="col d-flex flex-column justify-content-center align-items-center">
          {showingCreateNewQuestionForm === false &&
            questionCurrentlyBeingUpdated === null && (
              <div>
                ...
                <div className="mt-5">
                  ...
                  <button
                    onClick={() => {
                      setShowingCreateNewQuestionForm(true);
                    }}
          ...
          {questionCurrentlyBeingUpdated !== null && (
            <QuestionUpdateForm
              question={questionCurrentlyBeingUpdated}
          ...
          <tbody>
            {questions.map((question) => (
              <tr key={question.questionId}>
                ...
                <td>
                  <button
                    onClick={() => setQuestionCurrentlyBeingUpdated(question)}
                  ...
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `Are you sure you want to delete the question titled "${question.title}"?`
                        )
                      )
  ...
  function onQuestionUpdated(updatedQuestion) {
    ...
    const index = questionsCopy.findIndex(
      (questionsCopyQuestion, currentIndex) => {
    ...
    alert(
      `Question successfully updated. After clicking OK, look for the question with the title "${updatedQuestion.title}" in the table below to see the updates.`
    );
  }
  ...
  function onQuestionDeleted(deletedQuestionQuestionId) {
    ...
    alert(
      `Question successfully deleted. After clicking OK, look at the table to see your question disappear.`
    );
  }
}
